classdef BoardShim
    ...
    methods(Static)
        ...
        function package_num_channel = get_package_num_channel(board_id, preset)
            ...
            exit_code = calllib(lib_name, task_name, board_id, preset, res);
            BoardShim.check_ec(exit_code, task_name);
            % Matlab counts array elements from 1 while MindRove Core counts from 0%
    ...
    methods
        ...
        function response = config_board(obj, config)
            ...
            switch upper(config)
                case 'EEG_MODE'
                    config_str = char([0, 0, 0, 0, 0]);
                case 'BEEP'
                    config_str = char([1, 0, 0, 0, 0]);
                case 'BOOP'
                    config_str = char([2, 0, 0, 0, 0]);
                case 'IMP_MODE'
                    config_str = char([4, 0, 0, 0, 0]);
                case 'TEST_MODE'
                    config_str = char([16, 0, 0, 0, 0]);
                otherwise
                    error('Unsupported config mode: %s', config);
            end
        
            lib_name = BoardShim.load_lib();
        
        
            % Pass config_str as a normal MATLAB char (automatically becomes const char*)
            % no way to understand how it works in matlab used this link
            % https://nl.mathworks.com/matlabcentral/answers/131446-what-data-type-do-i-need-to-calllib-with-pointer-argument-char%
            [exit_code, tmp, response] = calllib(lib_name, task_name, config_str, blanks(4096), 4096, obj.board_id, obj.input_params_json);
