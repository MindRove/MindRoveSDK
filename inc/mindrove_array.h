...
class MindRoveArray;
template <typename T, size_t Dim>
std::ostream &operator<< (std::ostream &os, const MindRoveArray<T, Dim> &array);
...
public:
    friend std::ostream &operator<<<> (std::ostream &out, const MindRoveArray<T, Dim> &array);
    ...
    MindRoveArray (const std::array<int, Dim> &size)
    ...
    explicit MindRoveArray (int size0)
        : length (size0)
        , size (make_array (size0))
        , stride (make_stride<1> (make_array (size0)))
        , origin (nullptr)
    {
        static_assert (Dim == 1, "This function is only for MindRoveArray<T, 1>");
        origin = new T[size0];
        memset (origin, 0, sizeof (T) * size0);
    }

    MindRoveArray (int size0, int size1)
        : length (size0 * size1)
        , size (make_array (size0, size1))
        , stride (make_stride<2> (make_array (size0, size1)))
        , origin (nullptr)
    {
        static_assert (Dim == 2, "This function is only for MindRoveArray<T, 2>");
        origin = new T[length];
        memset (origin, 0, sizeof (T) * length);
    }

    MindRoveArray (int size0, int size1, int size2)
        : length (size0 * size1 * size2)
        , size (make_array (size0, size1, size2))
        , stride (make_stride<3> (make_array (size0, size1, size2)))
        , origin (nullptr)
    {
        static_assert (Dim == 3, "This function is only for MindRoveArray<T, 3>");
        origin = new T[length];
        memset (origin, 0, sizeof (T) * length);
    }

    MindRoveArray (T *ptr, int size0)
        : length (size0)
        , size (make_array (size0))
        , stride (make_stride<1> (make_array (size0)))
        , origin (nullptr)
    ...
    MindRoveArray (T *ptr, int size0, int size1)
        : length (size0 * size1)
        , size (make_array (size0, size1))
        , stride (make_stride<2> (make_array (size0, size1)))
        , origin (nullptr)
    ...
    MindRoveArray (T *ptr, int size0, int size1, int size2)
        : length (size0 * size1 * size2)
        , size (make_array (size0, size1, size2))
        , stride (make_stride<3> (make_array (size0, size1, size2)))
        , origin (nullptr)
    ...
    MindRoveArray (const MindRoveArray &other)
    ...
    const MindRoveArray &operator= (const MindRoveArray &other)
    {
        length = other.length;
        size = other.size;
        stride = other.stride;
    ...
    MindRoveArray (MindRoveArray &&other)
        : length (other.length)
    ...
    const MindRoveArray &operator= (MindRoveArray &&other)
    {
        length = other.length;
        size = std::move (other.size);
        stride = std::move (other.stride);
    ...
    int get_size (int dim) const
    {
        if ((dim < 0) || (dim >= Dim))
        {
            throw MindRoveException (
                "invalid dim argument", (int)MindRoveExitCodes::INVALID_ARGUMENTS_ERROR);
        }
        return size[dim];
    }
    ...
    T &at (int index0)
    {
        ...
        {
            throw MindRoveException (
                "out of range", (int)MindRoveExitCodes::INVALID_ARGUMENTS_ERROR);
        }
        return origin[idx];
    }
    ...
    {
        static_assert (Dim >= 2, "MindRoveArray dimension bounds error");
    ...
    {
        static_assert (Dim >= 3, "MindRoveArray dimension bounds error");
